...
public class OfficeBuilding implements IBuilding, Serializable {
	...
	private class ListOfficeFloor implements Cloneable, Serializable {
		...
		private class OfficeFloorNode implements Cloneable, Serializable {
	...
	public int getFloorNumberByFloor(IFloor floor)
	{
		for (int i = 0; i<floors.size(); i++)
		{
			if ( floors.getOfficeFloor(i) == floor)
			{
				return i;
			}
		}
	...
	public int getPlacementNumberBySpace(ISpace placement)
	{
		...
		for (int f = 0; f < floors.size(); f++)
		{
			for ( int p = 0; p < floors.getOfficeFloor(f).getCountPlacement(); p++)
			{
				if (floors.getOfficeFloor(f).getPlacement(p) == placement)
				{
					return n;
				}
				n++;
	...
	{
		return new ArrayIterator<>(getArrayFloor());
	}
}
